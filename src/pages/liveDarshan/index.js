...
import * as LiveActions from "../../redux/actions/liveAction.js";

const Live = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const { liveData } = useSelector(state => state?.liveReducer);
    const [searchText, setSearchText] = useState('');
    const filteredData = DeepSearchSpace(liveData, searchText);

    const [walletModal, setWalletModal] = useState(false);
    const [userId, setUserId] = useState('');

    console.log("check live data::::", liveData);
    ...
    //* Datatable Column
    const columns = [
        { name: "S.No.", selector: (row) => liveData.indexOf(row) + 1, width: "80px", },
        { name: "Temple", selector: (row) => row?.TempleName ? row?.TempleName : 'N/A', width: "160px" },
        { name: "Description", selector: (row) => row?.Description, width: "200px" },
        { name: "Link", selector: (row) => row?.VideoLink || 'N/A', width: '120px' },
        ...
        { name: "From Aarati Time", selector: (row) => row?.fromTimeOfArti, width: "150px", centre: true },
        { name: "To Aarati Time", selector: (row) => row?.toTimeOfArti, width: "150px", centre: true },
        ...
        {
            name: 'Action',
            cell: row => <div style={{ display: "flex", gap: "10px", alignItems: "center" }} >
                ...
                <div onClick={() => dispatch(LiveActions.deleteLive({ customerId: row._id }))} style={{ cursor: "pointer" }}><DeleteSvg /></div>
            </div>,
            center: true,
        },
    ];
    ...
    return (
        <>
            <div style={{ padding: "20px", backgroundColor: "#fff", marginBottom: "20px", boxShadow: '0px 0px 5px lightgrey', borderRadius: "10px" }}>
                <DatatableHeading title={'Live Darshan'} data={liveData} url={'/liveDarshan/add-live'} />
