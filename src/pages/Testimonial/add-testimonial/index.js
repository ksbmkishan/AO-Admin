...
const AddTestimonial = () => {
    ...
    const [testimonialDetail, setTestimonialDetail] = useState({ name: stateData ? stateData?.name : '', rating: stateData ? stateData?.rating : '', description: stateData ? stateData?.description : '' });
    const [inputFieldError, setInputFieldError] = useState({ name: '', rating: '', description: '', images: '' });
    const [images, setImages] = useState({ file: stateData ? img_url + stateData?.image : '', bytes: '' });
    ...
    const handleValidation = () => {
        ...
        if (rating <= 0) {
            handleInputFieldError("rating", "Please Enter Valid Rating (Greater Than 0)")
            isValid = false;
        }
        if (!Regex_Accept_Number.test(rating)) {
            handleInputFieldError("rating", "Please Enter Valid Rating")
            isValid = false;
        }
        ...
        <>
            <div style={{ padding: "20px", backgroundColor: "#fff", marginBottom: "20px", boxShadow: '0px 0px 5px lightgrey', borderRadius: "10px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px", fontFamily: 'Philosopher', backgroundColor: "#fff" }}>
                    <div style={{ fontSize: "22px", fontWeight: "500", color: Color.black }}>Add Testimonial</div>
                ...
                <Grid container sx={{ alignItems: "center" }} spacing={3}>
                    ...
                    <Grid item lg={12} md={12} sm={12} xs={12} >
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                            <label style={{ color: "#000", marginBottom: "#000", fontSize: "14.5px", color: "grey" }}>Testimonial Description <span style={{ color: "red" }}>*</span></label>
                            <textarea
                                name='description'
