...
const Product = () => {
    ...
    const productColumns = [
        ...
        { name: 'Image', cell: row => <img src={row?.image ? img_url + row?.image : logo} alt="Profile" style={{ width: '50px', height: '50px', borderRadius: '50%' }} /> },
        {
            ...
            cell: row => <div style={{ display: "flex", gap: "20px", alignItems: "center" }} >
                <div onClick={() => navigate('/temple/edit-product', { state: { stateData: row } })} style={{ cursor: "pointer" }}><EditSvg /></div>
    ...
    return (
        <>
            <MainDatatable data={ProductData} columns={productColumns} title={'Temple Product'} url={'/temple/product/add-product'} />
