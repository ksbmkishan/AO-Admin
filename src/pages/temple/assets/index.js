...
const Assets = () => {
    ...
    const filteredData = DeepSearchSpace(templeAssetsData, searchText);

    //* DataTable Columns
    const columns = [
        { name: 'S.No.', selector: row => templeAssetsData?.indexOf(row) + 1, width: '80px' },
        ...
        { name: 'Created Date', selector: row => moment(row?.createdAt)?.format('DD-MMM-YYYY') || 'N/A' },
        {
            ...
            cell: row => <div style={{ display: "flex", gap: "20px", alignItems: "center" }} >
                <div onClick={() => navigate('/temple/assets/items', { state: { itemData: row?.items, assetsId: row?._id } })} style={{ cursor: "pointer" }}><ViewSvg /></div>
                <div onClick={() => dispatch(TempleActions?.deleteTempleAssets({ _id: row?._id }))} style={{ cursor: "pointer" }}><DeleteSvg /></div>
            </div >,
            width: "180px"
        },
    ];
    ...
    return (
        <>
            <div style={{ padding: "20px", backgroundColor: "#fff", marginBottom: "20px", boxShadow: '0px 0px 5px lightgrey', borderRadius: "10px" }}>
                <DatatableHeading title={'Assets'} data={templeAssetsData} url={'/temple/assets/add-assets'} />
