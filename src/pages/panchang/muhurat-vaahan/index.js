...
const MuhuratVaahan = () => {
  ...
  // Initial fetch
  useEffect(() => {
    dispatch(PanchangActions.getPanchangVaahan({ year: 2025 }));
  }, [dispatch]);
...
useEffect(() => {
  if (panchangVaahan) {
    setInputFieldDetail(prev => ({
      description: panchangVaahan.description
        ? RichTextEditor.createValueFromString(panchangVaahan.description, "html")
  ...
  const handleInputField = (e) => {
    ...
    dispatch(PanchangActions.getPanchangVaahan({ year: Number(value) }));
  };
